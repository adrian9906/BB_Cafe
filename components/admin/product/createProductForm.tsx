...
export function CreateProductForm({ className, productSize, dialog, setDialog, ...props }: CreateProductsFormProps) {
  ...
  const handleSubmit = async (data: FormData) => {
    try {
      ...
      const result = await CreatePrduct(data)
      if (result) {
    ...
    <div className={(className)} {...props}>
        ...
        <DialogContent className="sm:max-w-md max-h-[80vh] overflow-y-auto">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
              ...
              <DialogFooter>
                <Button type="button" variant="outline" onClick={() => setDialog(false)} disabled={isLoading}>
                  Cancelar
                </Button>
      ...
      {productSize === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-4">Comienza agregando tu primer producto al catálogo.</p>
          <Button onClick={() => setDialog(true)} className="bg-amber-600 hover:bg-amber-700">
            {isLoading && (
